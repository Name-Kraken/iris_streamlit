...
# use_column_width = True 画像を自動で合わせる事ができる

# チェックボックス チェックマークを押すと画像が表示される
if st.checkbox('Show Image'):
    img = Image.open('iris.jpg')
    st.image(img,caption = 'Iris' , use_column_width = True)

# セレクトボックス　1～10の値が選べるようになる
option = st.selectbox(
    '好きな数字を入力してください。',
    list(range(1, 11))
)
...
# テキスト入力による値の動的変更 入力した値がHPに表示される
# sidebarはサイドに表示することが出来る
text = st.sidebar.text_input('あなたの好きなスポーツを教えて下さい。')
'あなたの好きなスポーツ：' , text

# スライダーによる値の動的変更　スライダーで動かした値がHPに表示される
condition = st.sidebar.slider('あなたの今の調子は？', 0, 100, 50)
'コンディション：' , condition

# expander　画面の表示非表示を行う
...
# プログレスバーが100まで進むと'done'と表示されるようになる